...
export default function SetupOrder({ coas, linkedCoas }: SetupOrderParam) {
  ...
  async function updateAcc(prev: number, id: number, accId: number) {
    if (linkedCoas.data.items.filter((x) => x.accId === accId && x.id != id).length > 0) {
      alert('Tidak dapat memilih akun yang sama untuk transaksi Order.')
      return
    }
    ...
    if (prev === 0) {
      const acc = await linkedCoas.insert(newAcc)
      ...
      if (acc) {
        ...
        linkedCoas.data.insert(0, { ...newAcc, accId: acc })
      }
    } else {
      const acc = await linkedCoas.update(newAcc)
      ...
      if (acc) {
        ...
        linkedCoas.data.update(id, { ...newAcc, accId: acc })
      }
    }
  }
}
